...
internal object AgentPrompts {
    ...
    fun systemPrompt(
        ...
        val languageGuidance = languageGuidanceFor(projectLanguage)
        return buildString {
            appendAssistantIdentity(mode, languageGuidance)
            appendResponseFormat()
            appendAvailableTools(mode)
            appendOperatingRules(mode)
            appendProjectContext(instructions, activeFilePath, sourcePackagePrefix, projectOutline)
        }
    }

    private fun languageGuidanceFor(projectLanguage: String): String =
        when (projectLanguage) {
    ...
    private fun StringBuilder.appendAssistantIdentity(mode: ChatMode, languageGuidance: String) {
        when (mode) {
    ...
    private fun StringBuilder.appendAvailableTools(mode: ChatMode) {
    ...
    private fun StringBuilder.appendOperatingRules(mode: ChatMode) {
        appendLine("RULES:")
        when (mode) {
            ChatMode.AGENT -> appendAgentRules()
            ChatMode.ASK -> appendAskRules()
            ChatMode.PLAN -> appendPlanRules()
        }
        appendLine("- Never wrap the JSON in markdown fences or add prose outside the JSON object.")
    }

    private fun StringBuilder.appendAgentRules() {
    ...
    private fun StringBuilder.appendAskRules() {
    ...
    private fun StringBuilder.appendPlanRules() {
        appendLine("- You are read-only: only list_dir, read_file, and search are available. Never edit files.")
    ...
    private fun StringBuilder.appendProjectContext(
        instructions: String,
        activeFilePath: String?,

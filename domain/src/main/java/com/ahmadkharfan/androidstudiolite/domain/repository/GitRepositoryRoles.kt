...
/** Cloning, initialization, and observing the working state of a repository. */
interface GitLifecycleRepository {
    fun clone(url: String, destination: File, options: CloneOptions, credentials: GitCredentials?): Flow<CloneProgress>
...
/** Computing diffs between the worktree, index, commits, and unsaved buffers. */
interface GitDiffRepository {
    ...
    suspend fun diffCommitToParent(repoDir: File, commitId: String, path: String, force: Boolean = false): GitFileDiff
    suspend fun diffIndexToBuffer(repoDir: File, path: String, buffer: String): GitFileDiff
}

/** Staging and unstaging changes at the file or hunk level. */
interface GitStagingRepository {
...
/** Creating commits and configuring the commit author. */
...
/** Listing, creating, checking out, renaming, and deleting branches. */
interface GitBranchRepository {
...
/** Reading commit history, file history, blame, and shallow-clone depth. */
...
/** Listing, creating, deleting, and pushing tags. */
interface GitTagRepository {
...
/** Stashing and restoring uncommitted work. */
interface GitStashRepository {
...
/** History-integrating operations (merge, rebase, cherry-pick, revert), conflict resolution, and reset/clean. */
interface GitIntegrationRepository {
...
/** Managing remotes and synchronizing with them (fetch, push, pull, upstream tracking). */
...
interface GitSubmoduleRepository {
